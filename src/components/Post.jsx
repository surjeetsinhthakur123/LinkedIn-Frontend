...
import { Loader, MessageCircle, Send, Share2, ThumbsUp, Trash2, Edit2, Check, X } from "lucide-react";
...
const Post = ({ post }) => {
	...
	// Edit Post
	const { mutate: editPost, isPending: isUpdatingPost } = useMutation({
		mutationFn: async () => {
			const payload = { content: editedContent };
			if (editedImage) payload.image = editedImage;
	...
	const handleAddComment = (e) => {
		...
		setComments([
			...
			{
				content: newComment,
				user: { _id: authUser._id, name: authUser.name, profilePicture: authUser.profilePicture },
				createdAt: new Date(),
			},
		]);
	};
			...
			<div className='p-4'>
				<div className='flex items-center justify-between mb-4'>
					...
					{isOwner && (
						<div className='flex gap-3'>
							{isEditing ? (
								<>
									<button onClick={editPost} className='text-green-500 hover:text-green-700'>
										{isUpdatingPost ? (
				...
				{isEditing ? (
					<div className='space-y-3'>
						...
						{editedImage && <img src={editedImage} alt='Preview' className='rounded-lg w-full mb-2' />}
					</div>
				) : (
					<>
